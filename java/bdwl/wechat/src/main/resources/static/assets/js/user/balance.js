$(function () {
    $(".rec-list-box .one-item").click(function () {
        $(".rec-list-box .one-item").each(function () {
            $(this).removeClass("active");
        })
        $(this).addClass("active");
        $("#rechargeAmountId").val($(this).attr("value"));
    })
})
 ...
 * @param successUrl
 */
function recharge(paySuccessUrl){
    var rechargeAmountId = $("#rechargeAmountId").val();
    if(rechargeAmountId == "" || rechargeAmountId == undefined){
        layer.msg("请选择充值金额");
    }
    $.ajax(
        {
            ...
            url:RECHARGE_API,
            data:{
                'rechargeAmountId':rechargeAmountId
            },
            error:function(){
                layer.msg("系统繁忙");
            },
            success:function(data){
                if(data.code == 0){
                    orderPay(data.data.signPackage,data.data.payMessage,paySuccessUrl)
                }else{
                    layer.msg(data.msg);
                }
            }
