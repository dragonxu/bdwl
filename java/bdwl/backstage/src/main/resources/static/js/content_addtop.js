...
function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
    ...
    Wind.use("artDialog","iframeTools",function(){
        //art.dialog.open('/BackStage/Asset/swfupload?a=swfupload' + args + setting, {
        art.dialog.open('/BackStage/Asset/swfupload', {
        title: name,
        id: uploadid,
        width: '650px',
        height: '420px',
...
function flashupload2(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
    ...
    var setting = '&module=' + module + '&catid=' + catid ;
    Wind.use("artDialog","iframeTools",function(){
        art.dialog.open('/BackStage/Asset/swfupload2?a=swfupload2&subject='+authkey + args + setting, {
            title: name,
            id: uploadid,
            width: '650px',
            height: '420px',
...
function flashupload3(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
    var args = args ? '&args=' + args : '';
    var setting = '&module=' + module + '&catid=' + catid ;
    Wind.use("artDialog","iframeTools",function(){
        art.dialog.open('/BackStage/Asset/swfupload3?a=swfupload3&subject='+authkey + args + setting, {
            title: name,
            id: uploadid,
            width: '650px',
            height: '420px',
...
function change_images2(uploadid, returnid) {
    ...
    $.each(contents, function(i, n) {
        var ids = parseInt(Math.random() * 10000 + 10 * i);
        var filename = filenames[i].substr(0, filenames[i].indexOf('.'));
        str += "<li style='float:left;margin-right:15px;margin-bottom:15px;' id='image" + ids + "'><input title='双击查看' type='image' name='" + returnid + "_url' src='" + n + "' style='height:100px;' ondblclick='image_priview(this.value);' class='input image-url-input'> <input type='hidden' name='" + returnid + "_url' value='" + n + "' style='width:160px;' class='input image-alt-input' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <a href=\"javascript:flashupload('replace_albums_images', '图片替换','image"+ids+"',replace_image2,'10,gif|jpg|jpeg|png|bmp,0','','','')\">替换</a>  <a href=\"javascript:remove_div('image" + ids + "')\">移除</a> </li>";
    });
    $('#' + returnid).html(str);
}
...
function replace_image2(uploadid, returnid) {
    ...
    $("#"+returnid).find(".image-url-input").attr('src',contents[0]);
    var filename = filenames[0].substr(0, filenames[0].indexOf('.'));
    $("#"+returnid).find(".image-alt-input").val(contents[0]);
}
...
//缩图上传回调
function thumb_images3(uploadid, returnid) {
    //取得iframe对象
    var d = uploadid.iframe.contentWindow;
    //取得选择的图片
    var in_content = d.$("#att-status").html().substring(1);
    ...
    if ($('#' + returnid + '_preview2').attr('src')) {
        $('#' + returnid + '_preview2').attr('src', in_content);
    }
    $('#' + returnid).val(in_content);
}
function thumb_images2(uploadid, returnid) {
    ...
    if ($('#' + returnid + '_preview').attr('src')) {
        ...
        $('#' + returnid + '_preview').html(in_content);
    }
    $('#' + returnid).val(in_content);
}
